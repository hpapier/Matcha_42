...
class View extends Component {
  ...
  orderByTags = list => {
    const newList = list.map(item => ({ ...item, tagPond: this.checkTags(item) }));
  ...
  getListOfData = () => {
    const { userMatchList } = this.props;
    let dataTrimed = userMatchList.slice(0);
          ...
          return (
            <div id='lgi-complete-user-like'>
              <div id='lgi-complete-user-like-view-title'>
                <div id='lgi-complete-user-like-view-title-text'>Tous vos match</div>
...
const mapStateToProps = state => ({
  userMatchList: state.userMatchList,
